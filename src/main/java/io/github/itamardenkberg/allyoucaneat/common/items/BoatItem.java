...
public class BoatItem extends Item {
	...
	public BoatItem(Item.Properties properties, BoatEntity.Type typeIn) {
		super(properties);
		this.type = typeIn;
	}
	...
	public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
		...
		if (raytraceresult.getType() == HitResult.Type.MISS) {
			...
			List<Entity> list = worldIn.getEntities(playerIn,
					playerIn.getBoundingBox().expandTowards(vec3d.scale(d0)).inflate(1.0D), ENTITY_PREDICATE);
			if (!list.isEmpty()) {
			...
			if (raytraceresult.getType() == HitResult.Type.BLOCK) {
				BoatEntity boatentity = new BoatEntity(worldIn, raytraceresult.getLocation().x,
